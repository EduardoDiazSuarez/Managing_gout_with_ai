...
import { GoogleGenAI, Type, FunctionDeclaration } from "@google/genai"; // keep Type defs used earlier for schema shapes
...
if (GEMINI_API_KEY) {
  console.log("GEMINI_API_KEY detected. Initializing GoogleGenAI client...");
...
app.post("/api/gemini/chat", async (req, res) => {
  ...
  const langInstruction = language === 'zh'
    ? 'IMPORTANT: Respond fluently, professionally, and naturally in Simplified Chinese (简体中文). Use standard clinical and dietary terms (e.g., 尿酸, 痛风, 嘌呤, 降酸, 碱化尿液).'
    : language === 'es'
    ? 'IMPORTANT: Respond fluently and professionally in Spanish (Español).'
    : 'Respond in English.';
  ...
  try {
    if (aiClient) {
      ...
      const SYSTEM_INSTRUCTION_CHAT = `You are Gemi Coach, an empathetic expert rheumatologist assistant and Gout Care Coach. Provide evidence-backed guidance (PubMed, Mayo Clinic, Healthline). DO NOT perform any remote side-effects.
${langInstruction}
...
Be concise, clear, and professional.`;
        ...
        config: {
          ...
          responseSchema: {
            ...
            properties: {
              text: {
                type: Type.STRING,
                description: "Empathetic clinical response to the user with evidence-backed guidance."
              },
              toolCalls: {
                type: Type.ARRAY,
                description: "List of tools or actions to run based on the user request, or an empty list.",
                items: {
                  type: Type.OBJECT,
                  properties: {
                    name: {
                      type: Type.STRING,
                      description: "The name of the tool to invoke. Valid values: 'log-water', 'add-natural-food', 'log-ua', 'add-flare', 'resolve-flare', 'add-exercise', 'add-sleep'."
                    },
                    args: {
                      type: Type.OBJECT,
                      description: "Arguments for the tool matching its specified parameters."
                    }
                  },
                  required: ["name", "args"]
                }
    ...
    // Fallback to original OpenRouter Agent SDK
    const SYSTEM_INSTRUCTION_CHAT = `You are Gemi Coach, an empathetic expert rheumatologist assistant and Gout Care Coach. Provide evidence-backed guidance (PubMed, Mayo Clinic, Healthline). DO NOT perform any remote side-effects.
${langInstruction}
...
app.post("/api/gemini/food-analysis", async (req, res) => {
  ...
  const { foodQuery, language } = req.body;
  if (!foodQuery || typeof foodQuery !== 'string' || foodQuery.trim().length === 0) {
    return res.status(400).json({ error: "Missing foodQuery parameter" });
  }

  const langInstruction = language === 'zh'
    ? 'IMPORTANT: All descriptive fields (ratingExplanation, uricAcidImpact, safetyTips, lowPurineAlternatives) MUST be written in Simplified Chinese (简体中文). Keep purineRating strictly as "Safe", "Moderate", or "High".'
    : language === 'es' 
    ? 'IMPORTANT: All descriptive fields (ratingExplanation, uricAcidImpact, safetyTips, lowPurineAlternatives) MUST be written in Spanish (Español). Keep purineRating strictly as "Safe", "Moderate", or "High".' 
    : 'Respond in English.';
...
${langInstruction}
PURINE CLASSIFICATION STANDARDS:
- High Risk / High Purine (Strict Avoidance / Heavy Limitation): Red meats (Beef roast/steak/ground, Pork chops/ribs/loin, Venison, Lamb), Bacon, Organ meats (Liver, Sweetbreads, Kidneys), Beer and alcoholic drinks, Small oily fish (Sardines, Anchovies, Herring, Mackerel), Shellfish (Mussels, Scallops, Shrimp), High-fructose corn syrup sodas and concentrated meat gravies/broths.
- Moderate Risk / Moderate Purine (Portion-Controlled <100g): White fish (Cod, Tilapia, Halibut, Flounder, Haddock), Salmon, Tuna, Skinless chicken breast, Tofu/Soy, Oatmeal, Spinach, Mushrooms, Lentils, Beans, Cauliflower.
- Safe / Low Purine (Daily Recommended): Water, Tart cherries, Low-fat plain yogurt, Skim milk, Eggs, Batatas/Sweet potatoes, Blueberries, Brown rice, Celery, Cucumbers, Unsweetened cacao.`,
          responseMimeType: "application/json",
          responseSchema: {
            type: Type.OBJECT,
            properties: {
              ...
              safetyTips: {
                type: Type.ARRAY,
                items: { type: Type.STRING },
                description: "Exactly 3 actionable safety tips."
              },
              lowPurineAlternatives: {
                type: Type.ARRAY,
                items: { type: Type.STRING },
                description: "Exactly 3 low-purine diet alternatives."
              }
            },
    ...
    // Fallback to original OpenRouter logic
    const systemInstruction = `You are an expert clinical dietician specializing in rheumatology and gout disease management. Provide accurate, evidence-based nutrition advice.
${langInstruction}
PURINE CLASSIFICATION STANDARDS:
- High Risk / High Purine (Strict Avoidance / Heavy Limitation): Red meats (Beef, Pork, Venison, Lamb), Bacon, Organ meats (Liver, Sweetbreads), Beer, Small oily fish (Sardines, Anchovies, Herring), Shellfish (Shrimp, Mussels), High-fructose corn syrup sodas and concentrated meat broths.
- Moderate Risk / Moderate Purine (Portion-Controlled <100g): White fish (Cod, Tilapia, Halibut, Flounder, Haddock), Salmon, Tuna, Chicken breast, Tofu, Oatmeal, Spinach, Mushrooms, Lentils.
- Safe / Low Purine (Daily Recommended): Water, Tart cherries, Low-fat plain yogurt, Skim milk, Eggs, Batatas/Sweet potatoes, Blueberries, Brown rice, Celery.
Output a single JSON object matching this shape: { foodName: string, purineRating: 'Safe'|'Moderate'|'High', ratingExplanation: string, uricAcidImpact: string, safetyTips: string[3], lowPurineAlternatives: string[3] }. Do not include any extraneous text.`;
