...
export default function App() {
  ...
  // Modified: Implements a secure sandbox that allows logging water, foods (including custom food additions), flare-up journals, exercise, sleep, and uric acid readings.
  const handleExecuteSkill = (name: string, args: any): string | null => {
    try {
      const sanitizedName = (name || '').trim().toLowerCase().replace(/_/g, '-');

      // 1. Log Water
      if (sanitizedName === 'log-water') {
        let amount = 250; // default
        if (args && typeof args === 'object') {
          const parsedAmount = Number(args.amount);
          if (!isNaN(parsedAmount) && parsedAmount > 0) {
            amount = Math.min(parsedAmount, 2000); // cap at 2000ml for safety
          }
        }
        ...
        saveHydration(updated);
        triggerAlert(`Logged +${amount}ml water via Gemi Coach!`, 'success');
        return `Successfully logged +${amount}ml water intake. Water database updated.`;
      }

      // 2. Log or Add Food (Supports both Check-off consumption and custom additions)
      if (sanitizedName === 'add-natural-food' || sanitizedName === 'log-food') {
        if (!args || typeof args !== 'object' || !args.name) {
          return `Invalid food arguments. "name" is required.`;
        }

        const foodNameInput = String(args.name).trim();
        const foodNameLower = foodNameInput.toLowerCase();

        // Check if the input corresponds to any of the pre-seeded Recommended Foods
        const recommendedFoodsList = [
          { id: 'seed-food-lemon', keywords: ['lemon', 'juice', 'citric'] },
          { id: 'seed-food-yogurt', keywords: ['yogurt', 'dairy', 'probiotic', 'bulgaricus', 'thermophilus'] },
          { id: 'seed-food-batata', keywords: ['batata', 'sweet potato', 'sweet potatoes'] },
          { id: 'seed-food-cacao', keywords: ['cacao', 'cocoa', 'chocolate'] },
          { id: 'seed-food-sesame', keywords: ['sesame', 'seed', 'seeds'] }
        ];

        // Find match
        let matchedId: string | null = null;
        for (const rec of recommendedFoodsList) {
          if (
            rec.keywords.some(kw => foodNameLower.includes(kw)) ||
            foodNameLower.includes(rec.id)
          ) {
            matchedId = rec.id;
            break;
          }
        ...
        if (matchedId) {
          // It's a recommended food! Check off consumption for today
          const todayStr = new Date().toISOString().split('T')[0];
          ...
          const updated = naturalFoods.map((s) => {
            if (s.id === matchedId) {
              foundAndLogged = true;
              if (!s.takenDates.includes(todayStr)) {
                return {
                  ...s,
                  takenDates: [...s.takenDates, todayStr],
                };
              }
            }
            return s;
          ...
          if (foundAndLogged) {
            saveNaturalFoods(updated);
            triggerAlert(`Checked off "${foodNameInput}" as consumed today!`, 'success');
            return `Successfully logged today's consumption of recommended food "${foodNameInput}".`;
          }
        }

        // If it's a new, custom food: add it to the watchlist
        const sanitizeStr = (s: any) => String(s || '').replace(/<[^>]*>/g, '').trim();

        const newFood: NaturalFood = {
          id: 'food-' + Date.now(),
          name: sanitizeStr(args.name),
          servingSize: sanitizeStr(args.servingSize || '1 portion'),
          frequency: (sanitizeStr(args.frequency || 'Daily') as any),
          mechanism: sanitizeStr(args.mechanism || 'Tracked natural dietician supplement'),
          category: (sanitizeStr(args.category || 'Other') as any),
          takenDates: [],
          notes: sanitizeStr(args.notes || 'Added safely via clinical assistant.'),
        };

        const updated = [...naturalFoods, newFood];
        saveNaturalFoods(updated);
        triggerAlert(`Tracked: ${newFood.name} added to watchlist!`, 'success');
        return `Successfully added custom food "${newFood.name}" to your watchlist!`;
      }

      // 3. Log Uric Acid Log
      if (sanitizedName === 'log-ua' || sanitizedName === 'log-uric-acid') {
        const val = Number(args.value || args.uricAcid);
        if (isNaN(val) || val <= 0) {
          return `Invalid uric acid value. Must be a positive number.`;
        }
        const notes = args.notes || 'Logged via Gemi Coach';
        handleAddUALog({
          date: new Date().toISOString().split('T')[0],
          value: val,
          notes,
        });
        return `Successfully recorded Uric Acid Level: ${val} mg/dL.`;
      }

      // 4. Log Flare Onset (Flare-Up Journal)
      if (sanitizedName === 'add-flare' || sanitizedName === 'log-flare') {
        const joint = args.joint || args.location || 'Big Toe';
        const painLevel = Number(args.painLevel || args.severity || 5);
        const triggers = Array.isArray(args.triggers) 
          ? args.triggers 
          : typeof args.triggers === 'string' 
            ? args.triggers.split(',').map((t: string) => t.trim()).filter(Boolean) 
            : [];
        const remediesTaken = Array.isArray(args.remediesTaken) 
          ? args.remediesTaken 
          : typeof args.remediesTaken === 'string' 
            ? args.remediesTaken.split(',').map((r: string) => r.trim()).filter(Boolean) 
            : [];
        const notes = args.notes || 'Logged via Gemi Coach';
        ...
        return `Successfully logged active Gout Flare-Up at the ${joint} (Pain Severity ${painLevel}/10) in your Care Journal.`;
      }

      // 5. Resolve Active Flare
      if (sanitizedName === 'resolve-flare' || sanitizedName === 'resolve-active-flare') {
        const active = flareLogs.find((f) => f.status === 'active');
        if (!active) {
          return `No active flares found in your record to resolve.`;
        }
        handleResolveFlareLog(active.id);
        return `Successfully resolved your active Gout Flare-Up at the ${active.joint}. Remission logged.`;
      }

      // 6. Log Exercise
      if (sanitizedName === 'add-exercise' || sanitizedName === 'log-exercise') {
        const activityType = args.activityType || 'Walking';
        const duration = Number(args.duration || 30);
        const jointStrain = Number(args.jointStrain || 2);
        const remissionPhase = args.remissionPhase !== undefined ? Boolean(args.remissionPhase) : true;
        const notes = args.notes || 'Logged via Gemi Coach';
        ...
        return `Successfully logged ${duration} mins of ${activityType} exercise in your physical activity tracker.`;
      }

      // 7. Log Sleep & Recovery
      if (sanitizedName === 'add-sleep' || sanitizedName === 'log-sleep') {
        const hours = Number(args.hours || 8);
        const quality = args.quality || 'Good';
        const restlessJoints = Boolean(args.restlessJoints);
        const meditationCompleted = Boolean(args.meditationCompleted);
        ...
        return `Successfully logged ${hours} hours of ${quality} quality sleep.`;
      }

      console.warn(`[Unknown Action] Blocked action: ${name}`, args);
      return `Action "${name}" is not fully supported in the current version of Gemi Coach.`;
    } catch (e: any) {
      console.error('Skill execution error', e);
      return `Execution failed: ${e.message || e}`;
    }
  };
  ...
  const handleExportBackup = () => {
    ...
    triggerAlert(t('app.backupSuccess', 'Natural care journal downloaded as backup JSON.'), 'success');
  };
    ...
    reader.onload = (event) => {
      try {
        ...
        if (json.flareLogs && importedFoods && json.uricAcidLogs) {
          ...
          triggerAlert(t('app.importSuccess', 'Joint care records imported successfully!'), 'success');
        } else {
          throw new Error('Missing indices');
        }
      } catch (err) {
        triggerAlert(t('app.importError', 'Error decoding backup file. Make sure file format remains unmodified.'), 'error');
      }
    };
  ...
  // Menu Tabs definitions with dynamic internationalized labels
  const TABS = [
    { id: 'dashboard', label: t('tabs.dashboard', 'Overview Dashboard'), icon: Activity },
    { id: 'ai-coach', label: t('tabs.aiCoach', 'Gemi Coach'), icon: BookOpen },
    { id: 'scanner', label: t('tabs.scanner', 'Diet (AI Food Scanner)'), icon: Sparkles },
    { id: 'hydration', label: t('tabs.hydration', 'Hydration (Water tracker)'), icon: Droplet },
    { id: 'rest', label: t('tabs.rest', 'Monitoring Rest'), icon: Moon, badge: activeFlareExists ? t('tabs.activeBadge', 'Active') : undefined, textCol: activeFlareExists ? 'text-rose-600 font-bold' : '' },
    { id: 'exercise', label: t('tabs.exercise', 'Exercise & Mobility'), icon: StretchDollIcon },
    { id: 'natural-foods', label: t('tabs.naturalFoods', 'Natural Lowering Foods'), icon: Leaf },
    { id: 'low-purine', label: t('tabs.lowPurine', 'Low-Purine (Safe)'), icon: Leaf },
    { id: 'moderate-purine', label: t('tabs.moderatePurine', 'Moderate-Purine'), icon: Sparkles },
    { id: 'high-purine', label: t('tabs.highPurine', 'High-Purine (Avoid)'), icon: Flame },
    { id: 'cooking-best-practices', label: t('tabs.cooking', 'Cooking Best Practices'), icon: Sparkles },
    { id: 'uric-acid', label: t('tabs.smartwatch', 'Monitor in Real Time with Smart Watch'), icon: Watch },
    // Symptoms placed at the bottom of the sidebar
    { id: 'symptoms', label: t('tabs.symptoms', 'Symptoms'), icon: FootPainIcon },
  ];
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-45">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="font-bold text-sm tracking-tight text-slate-900 block font-sans">{t('app.title', 'Gout Companion')}</span>
              <span className="text-[10px] text-slate-400 font-bold tracking-wider uppercase block">{t('app.subtitle', 'Therapeutic Care')}</span>
            </div>
          </div>

          {/* Language selector & Backup data buttons */}
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              title={t('app.exportBackup', 'Export Backup')}
              id="btn_export"
            >
              <Download size={13} />
              <span className="hidden sm:inline">{t('app.exportBackup', 'Export Backup')}</span>
            </button>
            <label
              className="text-slate-500 hover:text-slate-800 font-semibold text-xs px-3 py-1.5 rounded-xl border border-slate-200 hover:bg-slate-50 transition cursor-pointer flex items-center gap-1.5"
              title={t('app.importBackup', 'Import Backup')}
            >
              <Upload size={13} />
              <span className="hidden sm:inline">{t('app.importBackup', 'Import Backup')}</span>
              <input
                type="file"
                accept=".json"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1 grid grid-cols-1 lg:grid-cols-4 gap-8 w-full" id="main_app_layout">
        ...
        <aside className="lg:col-span-1" id="sidebar_tabs">
          <div className="bg-white rounded-3xl p-4 border border-slate-100 shadow-sm sticky top-24 space-y-2">
            <span className="text-[10px] uppercase font-bold tracking-wider text-slate-400 px-3.5 block mb-2">{t('app.trackAndRestore', 'Track & Restore')}</span>
