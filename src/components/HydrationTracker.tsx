...
export default function HydrationTracker({ log, onUpdateWater, onResetWater, onSetTarget }: HydrationTrackerProps) {
  ...
  const PRESETS = [
    { label: language === 'zh' ? '小水杯' : language === 'es' ? 'Vaso Pequeño' : 'Small Glass', value: 250, desc: '8 oz' },
    { label: language === 'zh' ? '大水杯' : language === 'es' ? 'Vaso Grande' : 'Large Tumbler', value: 500, desc: '16 oz' },
    { label: language === 'zh' ? '运动水壶' : language === 'es' ? 'Botella Deportiva' : 'Sports Bottle', value: 750, desc: '24 oz' },
    { label: language === 'zh' ? '大容量水桶' : language === 'es' ? 'Jarra / Botellón' : 'Mega Jug', value: 1000, desc: '32 oz' },
  ];
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="font-sans font-semibold text-lg text-slate-800 flex items-center gap-2">
            ...
            {t('hydration.title', 'Daily Hydration')}
          </h2>
          <p className="text-xs text-slate-500 mt-1">{t('hydration.subtitle', 'Flush uric acid naturally with pure water')}</p>
        </div>
        <button
          onClick={onResetWater}
          className="text-slate-400 hover:text-slate-600 p-2 rounded-xl hover:bg-slate-50 transition cursor-pointer"
          title={language === 'zh' ? '重置今日记录' : language === 'es' ? 'Restablecer registro del día' : 'Reset daily log'}
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
        ...
        <div className="md:col-span-5 flex flex-col items-center justify-center">
          ...
          <p className="text-xs text-center text-slate-400 mt-4 italic">
            {percent >= 100 
              ? (language === 'zh' ? '🎉 已达成今日饮水目标！继续保持！' : language === 'es' ? '🎉 ¡Meta diaria de hidratación alcanzada! ¡Excelente!' : '🎉 Daily flushing goal achieved! Exceptional!') 
              : (language === 'zh' ? '💧 建议每日摄入至少 2,500 mL 纯水，促进尿酸排泄并预防结晶析出。' : language === 'es' ? '💧 Intenta beber al menos 2,500 mL al día para evitar la formación de cristales.' : '💧 Aim for at least 2,500 mL daily to prevent crystal precipitation.')}
          </p>
        </div>
        ...
        <div className="md:col-span-7 flex flex-col justify-between h-full gap-4">
          <div className="space-y-3">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">{t('hydration.presets', 'Quick Log Presets')}</h3>
          ...
          <form onSubmit={handleAddCustom} className="mt-2" id="custom_hydration_form">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">{t('hydration.custom', 'Log Custom Intake')}</h3>
          ...
          <div className="mt-4 border-t border-slate-100 pt-4">
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">
              {language === 'zh' ? '体重与个性化饮水目标' : language === 'es' ? 'Peso corporal y meta de agua' : 'Body weight & target'}
            </h3>
            <div className="flex items-center gap-2 mb-2 flex-wrap sm:flex-nowrap">
              <input
                ...
                placeholder={language === 'zh' ? '体重 (kg)' : language === 'es' ? 'Peso en kg' : 'Body weight kg'}
                className="w-36 px-3 py-2 bg-slate-50 border border-slate-200 rounded-2xl text-sm"
              ...
              <button
                ...
                className="bg-emerald-600 hover:bg-emerald-700 transition cursor-pointer text-white px-3 py-2 rounded-2xl text-xs font-semibold"
                id="btn_apply_body_weight"
              >
                {language === 'zh' ? '应用推荐 (30ml/kg)' : language === 'es' ? 'Aplicar (30ml/kg)' : 'Apply (30ml/kg)'}
              </button>
              <div className="text-xs text-slate-500 ml-1">
                {language === 'zh' ? `当前目标: ${log.target} ml` : language === 'es' ? `Meta actual: ${log.target} ml` : `Current target: ${log.target} ml`}
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                disabled={!lastAdded}
                className={`px-3 py-1.5 rounded-2xl text-xs font-semibold transition ${lastAdded ? 'bg-rose-600 hover:bg-rose-700 text-white cursor-pointer' : 'bg-slate-100 text-slate-400 cursor-not-allowed'}`}
                id="btn_undo_water"
              >
                {language === 'zh' ? `撤销上次添加 (+${lastAdded || 0}ml)` : language === 'es' ? `Deshacer último (+${lastAdded || 0}ml)` : `Undo last (+${lastAdded || 0}ml)`}
              </button>
              ...
              >
                {language === 'zh' ? '清除体重' : language === 'es' ? 'Borrar peso' : 'Clear weight'}
              </button>
            </div>
          </div>
        </div>
