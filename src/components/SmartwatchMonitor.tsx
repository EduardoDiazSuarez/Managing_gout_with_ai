      ...
      <div className="bg-gradient-to-r from-violet-600/10 to-indigo-600/5 rounded-3xl p-6 border border-violet-500/10 flex flex-col md:flex-row md:items-center gap-6 justify-between">
        <div className="space-y-2">
          <span className="bg-violet-100 text-violet-800 text-[10px] font-bold font-mono px-2.5 py-1 rounded-full uppercase tracking-wider block w-fit">
            {language === 'zh' ? '新一代可穿戴传感技术' : language === 'es' ? 'Tecnología Portátil de Última Generación' : 'Next-Gen Wearable Tech'}
          </span>
          <h1 className="font-sans font-bold text-2xl text-slate-800 flex items-center gap-2">
            ...
            {language === 'zh' ? '智能手表实时尿酸监测' : language === 'es' ? 'Sincronización de Ácido Úrico con Smartwatch' : 'Smartwatch Real-Time Uric Acid Sync'}
          </h1>
          <p className="text-sm text-slate-600 max-w-2xl leading-relaxed">
            {language === 'zh'
              ? '持续监测局部关节生理指标、皮肤温度变化与经皮汗液生物传感器数值，实时掌握尿酸盐代谢状况。'
              : language === 'es'
              ? 'Monitorea continuamente estadísticas articulares locales, picos de temperatura de la piel y métricas de biosensor de sudor para mapear cristales en tiempo real.'
        ...
        >
          <RefreshCw size={14} />
          {language === 'zh' ? '校准 / 模拟传感器' : language === 'es' ? 'Calibrar / Simular Sensor' : 'Calibrate/Simulate Node'}
        </button>
      </div>
        ...
        >
          <div className="flex items-center gap-2 pb-4 mb-4 border-b border-slate-50">
            ...
            <h3 className="font-sans font-bold text-sm text-slate-700">
              {language === 'zh' ? '校准局部关节生物传感器' : language === 'es' ? 'Calibrar Biosensor Articular' : 'Calibrate Local Joint Biosensor'}
            </h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div>
              <label className="text-xs font-bold text-slate-500 block mb-1.5">
                {language === 'zh' ? '输入临床血液化验参考值 (mg/dL) *' : language === 'es' ? 'Valor de Referencia de Laboratorio (mg/dL) *' : 'Enter Laboratory Reference / Blood Test Value (mg/dL) *'}
              </label>
              <input
                type="number"
                step="0.1"
              ...
              <p className="text-[10px] text-slate-400 mt-1">
                {language === 'zh' ? '利用准确的静脉血检基线校准智能手表的透皮分析系数。' : language === 'es' ? 'Calibra el coeficiente transdérmico usando una muestra precisa de sangre.' : 'Calibrates the smartwatch transdermal coefficient utilizing an accurate dry blood sample baseline.'}
              </p>
            </div>
            <div>
              <label className="text-xs font-bold text-slate-500 block mb-1.5">
                {language === 'zh' ? '校准备注' : language === 'es' ? 'Notas de Calibración' : 'Calibration Notes'}
              </label>
              <input
                type="text"
                placeholder={language === 'zh' ? '例如：与医院早晨空腹抽血化验单对齐' : language === 'es' ? 'ej. Sincronizado con análisis de sangre' : 'e.g. Sychronized with lab test baseline'}
                value={simNotes}
          ...
          <div className="flex gap-2 justify-end">
            ...
            >
              {language === 'zh' ? '确认校准' : language === 'es' ? 'Confirmar Calibración' : 'Confirm Calibration'}
            </button>
          </div>
        </form>
